...
/*===============================================*/
/*timer resource , time 1, 双OC interrupt   FOR UART tx/rx */
...
#define asUART_TOP_DEFAULT	100  //在出现错误的时候自动将内部的计数定为此值。

#define asUART_TIMER_TOP  OCR1C
...
#define T0val	0        /*到255溢出共255*8us = 2.04ms*/
...
/*
  *通讯步骤
  *      1. 上位机设置好参数后发送0X80,
  *          如果检测成功下位机会发送一个检测出的参数，
  *          否则没有任何数据返回，上位机就要重复发送0X80；
  *      
  *      2. 在接收到数据后，上位机要发送0X55,以检验通讯正确性，
  *          下位机在检测正确后会返回0X55，否则返回的值不是0X55。
  *          要重新返回发送0X80.
  */

  /*  
    * 程序采用了自动设置波特率的方式，并且实现了全双工通讯，
    * 实际检测实验在4800bps--19200bps连续收发完全无误！
    * 测试过程中传送的数据是15KBytes。
    */

  /*
    *  控制方式改变如下：
    *  主要控制放在定时器1中，且定时器1不停止，看门狗的复位也放在
    *  这里面。
    */

 /* 波特率自动检测
   *
   * 实际工作过程中，上位机发送0x80后，单片机会进行检测并在同步后
   * 向上位机发送一个内部检测出来的值。
   *
    * 检测串行波特率的时候，用定时器0来定时。其余时候定时器0不工作。
   * 将UART 收发周期在asUART_TIMER_TOP 寄存器中
   *
   *  UART 收发
   *
   *不用移位，也就是不用USI功能，移位由软件来实现，输入放在INT0上，
   *输出放在OC1B处，保证输出时钟准确。
   *
   *  由OCR1B来输出串行输出部分，当发生匹配的时候自动改变输出电平，
   *  同时引发中断，在中断中设置下次匹配时的输出电平。
   *  并且能够检测出是否发送完成，形成相应的标志，
   *  
   *  OCR1A设置在接收到启动位时自动设置。看门狗可以在这里清除。
   *  OCR1A不对外输出，主要用来检测外部的输入。但是启动位由程序主动
   *  来检测，当检测到启动位后会对OCR1A进行自动计算设置，
   */

/*
  *  单片机的工作时钟一般要达到4M以上，现在决定采用8M。
  *  状态指示部分由OCR1B来实现，如果没有串行通讯就闪烁，否则就常亮。
  *  并且闪亮的速度与实际同步后的速度相成比例。
  */


    
/* 我自己在试验中发现，采用内部振荡器时，实际的振荡频率比标称
  * 值大，所以所测出的值会大于我们理论计算出来的值。
  */
